...
// function createCardQuery(Card cardParams) returns string {
//     string cardQuery = "";
//     foreach [string, anydata] [key, value] in cardParams.entries() {
//         if (key == "number") {
//             cardQuery = "card[number]=" + value.toString() + "&"; 
//         } else {
//             cardQuery = cardQuery + "card[" + key + "]=" + getEncodedUri(value.toString()) + AND;
//         }
//     }
//     return cardQuery;
// }
...
function mapToCardRecord(http:Response response) returns @tainted Card|Error {
    json|error payload = response.getJsonPayload();
    if (payload is error) {
        return setJsonResError(payload);
    } else {
        ...
        io:println("---------------------------------");
        check checkForErrorResponse(payload);
        Card|error card = Card.constructFrom(payload);
        if (card is error) {
            return Error(message = "Response cannot be converted to Card record", cause = card);
        } else {
            return card;
        }
    }        
